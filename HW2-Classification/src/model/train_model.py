...
def train_model(dataset_path, model):
    ...
    for epoch in range(num_epochs):
        ...
        writer.add_scalar("Train/Accuracy", train_acc/len(train_loader.dataset), epoch)
        writer.add_scalar("Validation/Accuracy", val_acc/len(val_loader.dataset), epoch)

        print(f"Epoch [{epoch + 1}/{num_epochs}]: Train Acc: {train_acc/len(train_loader.dataset):.5f} Loss: {train_loss/len(train_loader):.5f} | Val Acc: {val_acc/len(val_loader.dataset):.5f} loss: {val_loss/len(val_loader):.5f}\n")
